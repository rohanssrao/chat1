...
window.startApp = function () {
    ...
    var scrollToMessage = function (scrolled, isOwnSender) {
        if (isOwnSender === void 0) { isOwnSender = false; }
        if (scrolled) {
            $('#chat').animate({
    ...
    function isYouTube(url) {
        if (url !== undefined || url !== '') {
            var match = url.match(/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=|\?v=)([^#\&\?]{11})$/);
            if (match) {
